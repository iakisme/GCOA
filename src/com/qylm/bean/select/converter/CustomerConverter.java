...
public class CustomerConverter implements Converter {
	...
	public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
		if (value != null && value.trim().length() > 0) {
			try {
				CustomerService service = ApplicationContextHelper.getBean("customerService");
				return service.getById(Integer.parseInt(value));
			} catch (NumberFormatException e) {
				throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "选择错误", "没有此客户！"));
			}
		} else {
			return null;
		}
