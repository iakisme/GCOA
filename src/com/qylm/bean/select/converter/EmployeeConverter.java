...
public class EmployeeConverter implements Converter {
	...
	public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
		if (value != null && value.trim().length() > 0) {
			try {
				EmployeeService service = ApplicationContextHelper.getBean("employeeService");
				return service.getById(Integer.parseInt(value));
			} catch (NumberFormatException e) {
				throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "选择错误", "没有此员工信息！"));
			}
		} else {
			return null;
		}
