...
public class UserManageBean extends BasePagingBean {
	...
	protected void fetchData(int start, boolean needInit) {
		...
		// 如果不是超级系统管理员，就只能查询本公司的数据情况
		if (User.USER_LEVEL_1.equals(userBean.getUser().getUserlevel())) {
			detachedCriteria.add(Restrictions.eq(User.CREATER, userBean.getUser()));
			...
			detachedCriteria.add(Restrictions.eq(User.BELONGING_USER, userBean.getUser().getBelongingUser()));
		}
